...
class TmxToVttPanel(tkinter.Frame):
    def __init__(self, parent, *args, **kwargs):
        super(TmxToVttPanel, self).__init__(parent, *args, **kwargs)
        self.parent = parent

        # -- Configure invisible items
        self.string_var_selected_radio = tkinter.StringVar()
        self.string_var_single_file = tkinter.\
            StringVar(self.parent, value='Path to tmx file ...')
        self.string_var_multiple_files = tkinter.\
            StringVar(self.parent, value='Path to directory ...')

        # -- Configure row 1
        row = 0
        column = 0
        self.radio_single_file = Radiobutton(self.parent,
                                             text='Single File',
                                             indicator=1,
        ...
        self.textbox_single_file = tkinter. \
            Entry(self.parent, textvariable=self.string_var_single_file)
        self.textbox_single_file.grid(row=row, column=column)
        column += 1
        ...
        row += 1
        column = 0
        self.radio_multiple_files = Radiobutton(self.parent,
                                                text='Multiple Files',
                                                indicator=1,
        ...
        self.textbox_multiple_files = tkinter.\
            Entry(self.parent, textvariable=self.string_var_multiple_files,
                  state=tkinter.DISABLED)
        self.textbox_multiple_files.grid(row=row, column=column)
        column += 1
